...
def _infer_intent(el_type, text, purpose):
    ...
    if el_type in ("external_link","exit"):
        return "exit"
    if any(k in t for k in ["buy","order","checkout","purchase","enroll","register",
                              "get started","trial","demo","request","book",
                              "virtual appt","talk to a doctor","sign up","copay",
                              "savings","find a doctor","locate","patient support","schedule"]):
        return "conversion"
    if any(k in t for k in ["subscribe","newsletter","join"]):
        return "acquisition"
...
def _make_event_name(el_type, text, purpose):
    s = re.sub(r"[^a-z0-9]+","_",(text or "").lower().strip()).strip("_")[:40]
    if el_type == "form":        return f"form_submit_{purpose}" if purpose else "form_submit"
    if el_type == "form_field":  return f"form_field_{s}"
    if el_type == "download":    return f"file_download_{s[:30]}"
    if el_type in ("link","navigation"): return f"click_{s}"
    if el_type == "external_link": return f"exit_click_{s}"
    if el_type == "button":      return f"cta_click_{s}"
    if el_type == "video":       return "video_play"
    return f"interaction_{s}"


def _fix_category(category, intent, text):
    t = text.lower()
    if intent == "conversion": return "conversion"
    if intent == "acquisition" and category == "navigation": return "conversion"
    if any(k in t for k in CONVERSION_CTA_KEYWORDS): return "conversion"
    return category
...
def classify_elements(elements, options=None):
    ...
    for el in elements:
        ...
        if el_type == "form":
            category, action, label = ("conversion" if intent=="conversion" else "form_interaction"), "submit", f"{purpose}_form" if purpose else "form"
        elif el_type == "form_field":
            category, action, label = "form_interaction", "field_interaction", text
        elif el_type == "download":
            category, action, label = "download", "click", el.get("href",text).split("/")[-1][:80]
        elif el_type == "external_link":
            category, action, label = "exit", "click", el.get("href",text)[:80]
        elif el_type == "button":
            category, action, label = "ui_interaction", "click", text[:80]
        elif el_type == "video":
            category, action, label = "engagement", "play", text[:80]
        else:
            category, action, label = "navigation", "click", text[:80]
...
def _enrich_chunk(chunk, api_key, provider, model):
    prompt = (
...
def enrich_with_llm(rows, api_key, provider):
    """Enrich ALL rows in batches of 20 — fixes the old rows[:20] cap."""
    if not api_key or not rows:
        return rows
    model = "llama3-8b-8192" if provider == "groq" else "gpt-4o-mini"
    for i in range(0, len(rows), 20):
        try:
            rows[i:i+20] = _enrich_chunk(rows[i:i+20], api_key, provider, model)
        except Exception as e:
            print(f"[LLM] Batch {i//20+1} failed: {e}")
    return rows
...
def generate_isi_events(elements):
    ...
    if not has_isi:
        return []
    return [
        {"Category":"engagement","Event Name":f"scroll_isi_{p}","Action":"scroll",
         "Label":f"ISI Section {p}%","Business Intent":"awareness",
         "Element Type":"scroll","Source":"auto_isi","Notes":"Auto-generated: ISI detected"}
        for p in [25,50,75,100]
    ]


def get_pharma_template(name):  return PHARMA_TEMPLATES.get(name, [])
def list_pharma_templates():    return list(PHARMA_TEMPLATES.keys())
